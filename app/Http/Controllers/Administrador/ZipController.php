...
class ZipController extends Controller
{
    ...
    public function downloadFolder(Request $request)
    {
        ...
        $filesInSubprocesses = glob(storage_path('app/public/'.$proceso->codigo.'/*/*/*.*'));
        $filesNotInSubprocesses = glob(storage_path('app/public/'.$proceso->codigo.'/*/*.*'));

        $files = array_merge($filesInSubprocesses, $filesNotInSubprocesses);

        if(empty($files)){
            return back()->With('error', 'El proceso no cuenta con archivos internos.');
        }
    ...
    public function downloadFolderSubproceso(Request $request)
    {
        ...
        if(empty($files)){
            return back()->With('error', 'El subproceso no cuenta con archivos internos.');
        }
    ...
    public function downloadFolderProcesoPersonal(Request $request)
    {
        ...
        // Obtenemos los archivos de la carpeta del subproceso seleccionado
        try { // Si esto se rompe, significa que el proceso personal no esta en un subproceso
            $files = glob(storage_path('app/public/'.$procPers->subproceso->proceso->codigo.'/'.$procPers->subproceso->codigo.'/'.$procPers->codigo.'/*.*'));    
        } catch (\Throwable $th) {
            $files = glob(storage_path('app/public/'.$procPers->proceso->codigo.'/'.$procPers->codigo.'/*.*'));
        }
    ...
    {
        try { // Si esto se rompe, significa que el documento no esta en una carpeta de subproceso
